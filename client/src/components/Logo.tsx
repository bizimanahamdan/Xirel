 ...
 * clay-rust gradient with a sage ribbon accent.
 */
export function Logo({ variant = "full", size = 36, textClassName = "", className = "" }: LogoProps) {
  ...
  return (
    <span className={`inline-flex items-center gap-2.5 ${className}`}>
      ...
      >
        <defs>
          <linearGradient id={gradientId} x1="14" y1="14" x2="86" y2="86" gradientUnits="userSpaceOnUse">
            <stop offset="0" stopColor="#B5602B" />
            <stop offset="1" stopColor="#E08B4E" />
          </linearGradient>
      ...
      {variant === "full" && (
        <span
          className={`gradient-text font-semibold ${textClassName}`}
